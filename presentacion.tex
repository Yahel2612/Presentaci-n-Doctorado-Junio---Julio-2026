...
%\include{Actividades durante el semestre}
...
\begin{frame}{Espacios de funciones vectoriales sobre ENOs}
\begin{block}{}
...
\begin{frame}{}
  ...
  \begin{block}{}
      Sea $ (X,|| \cdot || ) $ un espacio de Riesz. Una función  $f : [a,b] \longrightarrow X $ es de \textbf{variación acotada respecto al orden}, si existe  $M \in X $ tal que,
...
\begin{frame}{}
    El siguiente ejemplo es de una función creciente que es de variación acotada débil, pero no es de variación acotada fuerte.
...
\begin{frame}{No todas las funciones crecientes son de variación acotada fuerte o de variación acotada débil}
    \begin{itemize}
        \item \textbf{Ejemplo:} 
        \begin{block}{}
            Sean $X=C_{\mathbb{R}}^{1}[0,1]$, $||f|| = ||f||_{\infty} + ||f'||_{\infty}$ y $P_0 = \{f \in X: f(0)=0 \  y  \  f(t)\geq 0, \forall t \in[0,1]\}$.\\  $( X, ||\cdot ||, P_0)$ es un ENO.
        \end{block}
        \item Se probará que existe una función creciente que no es de variación acotada débil y por lo tanto no es de variación acotada fuerte.
...
\textbf{Lema:} Sea $X$ un ENO. Son equivalentes
    \begin{itemize}
        ...
        \item Si toda sucesión creciente $ \{ x_{n} \} $ en $X$ converge, si y sólo si existe $ \{ x_{n_{k}} \} $ una subsucesión de $ \{ x_{n} \} $ convergente.
...
\begin{frame}{Trabajo a futuro}
    ...
    \begin{block}{Definición:}
        ...
        dado $ \epsilon > 0  $, existe $ \delta >0  $ tal que para toda partición $ \{ (I_{i} , \xi_{i} ) : i = 1,..., n \}   $ de $ [a,b]  $ con norma menor que $  \delta  $, se cumple que
        \begin{equation*}
...
\begin{thebibliography}{99}
	...
	Nova Science Publishers Inc. Segunda edición.\hypertarget{05}{ (2004)}.
...
Sea $X$ un ENO  donde $ X_{+} $ es normal y $ E^{1}  = C^{1} ( [a,b],X) $. Se define $E_{+}^{1} = \{ f \in E^{1}: f(t) \geq 0, f'(t) \geq 0 \ \forall t \in [a,b]  \} $ y $ || f ||_{\infty} = \sup \{ || f(t) ||_{X} : t \in [a,b] \} $.\\
      Propiedades:
    \begin{enumerate}
        \item Si $X_{+}$ es normal, entonces $E_{+}^{1}$ es normal.
        \item Si en $X$ todos los intervalos ordenados cerrados son compactos, entonces en $E^{1}$ todos los intervalos ordenados cerrados son compactos.
        \item Dependiendo su orden es normal o no. 
    \end{enumerate}
\end{frame}
